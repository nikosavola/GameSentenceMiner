...
class Store:
    ...
    def get(self, key: str, default: Any = None, *, copy: bool = True) -> Any:
        # copy=False returns the live value without a deepcopy; callers MUST treat
        # the result as read-only. Safe only for scalar getters (per-frame OCR path).
...
def _get_ocr_value_readonly(key: str, default: Any = None) -> Any:
    ...
    # Result MUST be treated as read-only; only use for scalar values.
